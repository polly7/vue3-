...
function request(axiosConfig, customOptions = {}, loadingOptions = undefined) {
    const service = axios.create({
        baseURL: process.env.VUE_APP_BASE_API, // 设置统一的请求前缀
        timeout: 10000 * 12 // 设置统一的超时时长
    })
    ...
    service.interceptors.request.use(
        config => {
            ...
            // 创建loading实例
            if (custom_options.loading && loadingOptions) {
...
function httpErrorStatusHandle(error) {
    ...
    if (axios.isCancel(error)) {
        // return console.info('请求的重复请求：' + error.message)
        return
    }
    let message = ''
    if (error && error.response) {
        ...
        switch (error.response.status) {
            case 302:
                ...
                break
            case 400:
                message = error.response.data.msg || '参数不正确！'
                break
            case 401:
    ...
    // if (error.response.data.code !== 20000000) {
    //   Message({
    //     type: 'error',
    //     message
    //   })
...
export const oauthRequest = axios.create({
    baseURL: process.env.VUE_APP_OAUTH_URL, // process.env.VUE_APP_BASE_API
